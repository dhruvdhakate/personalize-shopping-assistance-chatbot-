...
def extract_budget(text):
    ...
    match = re.search(r'(\d+(?:\.\d+)?)\s*(k|thousand|lakh)?', text)
...
def chat():
    ...
    if not message:
        return jsonify({
            "reply": "Please tell me what you want to shop for.",
            "products": []
        })
    ...
    if category and budget:
        reply = (
            f"I found {len(results)} {category} options "
            f"around your budget of ₹{budget:,.0f}. "
            "Here are my best picks."
        )

    elif category:
        reply = (
            f"Here are some highly rated {category} options. "
            "I can also filter them by your budget."
        )

    elif budget:
        reply = (
            f"Here are highly rated products close to your "
            f"₹{budget:,.0f} budget."
        )

    else:
        reply = (
            "Sure! Here are some popular picks. "
            "Try messages like 'best phone under 50000'."
        )
